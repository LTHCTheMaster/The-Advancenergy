# Tin Armor and Tools
execute if score #found furnace_nbt_recipes.data matches 0 store result score #found furnace_nbt_recipes.data if data storage furnace_nbt_recipes:main input.tag.ctc.traits{"metal/tin":1b,"equipment":1b} run loot replace block ~ ~ ~ container.3 loot lthc.advancenergy:i/ores/tin/nugget_x2

# Lead Armor and Tools
execute if score #found furnace_nbt_recipes.data matches 0 store result score #found furnace_nbt_recipes.data if data storage furnace_nbt_recipes:main input.tag.ctc.traits{"metal/lead":1b,"equipment":1b} run loot replace block ~ ~ ~ container.3 loot lthc.advancenergy:i/ores/lead/nugget_x2

# Silver Armor and Tools
execute if score #found furnace_nbt_recipes.data matches 0 store result score #found furnace_nbt_recipes.data if data storage furnace_nbt_recipes:main input.tag.ctc.traits{"metal/silver":1b,"equipment":1b} run loot replace block ~ ~ ~ container.3 loot lthc.advancenergy:i/ores/silver/nugget_x2

# Bronze Armor and Tools
execute if score #found furnace_nbt_recipes.data matches 0 store result score #found furnace_nbt_recipes.data if data storage furnace_nbt_recipes:main input.tag.ctc.traits{"metal/bronze":1b,"equipment":1b} run loot replace block ~ ~ ~ container.3 loot lthc.advancenergy:i/materials/bronze_nugget_x2
