...
execute as @e[tag=itemio.selected.input, tag=!itemio.container.auto_handled_io, tag=!itemio.container.nope] run function itemio:v0.2.0/container/input/custom/input
execute unless entity @e[tag=itemio.selected.input, tag=!itemio.container.nope, tag=!itemio.container.auto_handled_io] if block ~ ~ ~ #itemio:container run function itemio:v0.2.0/container/input/vanilla/test_input
